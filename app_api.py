...
def get_all_available_cameras():
    """Returns a list of all available camera indices, avoiding locking the active camera."""
    available = []
    # If the camera is currently active, we know it exists
    if camera_connected and LAPTOP_CAMERA_INDEX is not None:
        available.append(LAPTOP_CAMERA_INDEX)
        
    for idx in [0, 1, 2, 3, 4]:
        if idx == LAPTOP_CAMERA_INDEX and camera_connected:
            continue
...
def detect_best_camera_index():
    """Auto-detect the best webcam index. Prefer external USB webcam (highest index) over built-in."""
    available = get_all_available_cameras()
    if not available:
        print("⚠️ No cameras detected, defaulting to index 0")
        return 0
    # If multiple cameras found, prefer the highest index (usually the external USB webcam)
    best = available[-1] if len(available) > 1 else available[0]
    print(f"📷 Available cameras: {available}. Using index {best} {'(external USB)' if best > 0 else '(built-in)'}")
    return best

LAPTOP_CAMERA_INDEX = detect_best_camera_index()
...
def webcam_reader():
    """Continuously read frames from the laptop's built-in webcam.
    When USE_LAPTOP_CAMERA is True and there is an active viewer, the camera stays open.
    Otherwise, it is released to respect privacy and resource usage."""
    global latest_frame, latest_frame_time, camera_connected, active_video_connections, force_camera_active
    cap = None
    current_cap_index = None
    consecutive_failures = 0

    while True:
        # Release and sleep if webcam mode is disabled OR there are no active video feeds
        if not USE_LAPTOP_CAMERA or (active_video_connections <= 0 and not force_camera_active):
            if cap is not None and cap.isOpened():
                print("🔌 Releasing laptop camera (switched source or stream inactive)...")
                cap.release()
                cap = None
                current_cap_index = None
            camera_connected = False
        ...
        # Open camera if not already open or if target index changed
        if cap is None or not cap.isOpened() or current_cap_index != LAPTOP_CAMERA_INDEX:
            if cap is not None:
                print(f"🔄 Releasing camera at index {current_cap_index} to switch to index {LAPTOP_CAMERA_INDEX}...")
                cap.release()
                cap = None

            # Try the detected best index first
            cap = cv2.VideoCapture(LAPTOP_CAMERA_INDEX)
            if not cap.isOpened():
                print(f"❌ Could not open camera at index {LAPTOP_CAMERA_INDEX}. Scanning others...")
                found = False
                for alt_idx in [0, 1, 2, 3, 4]:
                    if alt_idx == LAPTOP_CAMERA_INDEX:
                        continue
                    temp_cap = cv2.VideoCapture(alt_idx)
                    if temp_cap.isOpened():
                        ret, test_frame = temp_cap.read()
                        if ret and test_frame is not None:
                            print(f"✅ Found working webcam at index {alt_idx}!")
                            cap = temp_cap
                            found = True
                            current_cap_index = alt_idx
                            break
                        temp_cap.release()
                if not found:
                    print("❌ No working webcam found, retrying in 2s...")
                    camera_connected = False
                    clear_latest_frame()
                    current_cap_index = None
                    time.sleep(2)
                ...
                print(f"✅ Camera opened at index {LAPTOP_CAMERA_INDEX}")
                current_cap_index = LAPTOP_CAMERA_INDEX
            consecutive_failures = 0
        ...
        if not ret or frame is None:
            ...
            if consecutive_failures >= WEBCAM_MAX_CONSECUTIVE_FAILURES:
                print("⚠️ Too many read failures, reconnecting camera...")
                camera_connected = False
                clear_latest_frame()
                cap.release()
                cap = None
...
if is_frozen:
    # Always start threads in frozen executable mode
...
def video_feed():
    global active_video_connections, LAPTOP_CAMERA_INDEX
    # Run dynamic webcam detection when user initiates /video_feed stream
    LAPTOP_CAMERA_INDEX = detect_best_camera_index()
...
def set_camera_source():
    global USE_LAPTOP_CAMERA, LAPTOP_CAMERA_INDEX
    source = request.form.get("source")
    if source == "built-in":
        USE_LAPTOP_CAMERA = True
        LAPTOP_CAMERA_INDEX = 0
        clear_latest_frame()
        print("📷 Camera source set to Built-in Camera (Index 0)")
        return jsonify({"status": "success", "use_laptop_camera": True, "active_index": 0})
    elif source == "external":
        USE_LAPTOP_CAMERA = True
        available = get_all_available_cameras()
        external_cameras = [idx for idx in available if idx > 0]
        if external_cameras:
            LAPTOP_CAMERA_INDEX = external_cameras[0]
        else:
            LAPTOP_CAMERA_INDEX = 3
        clear_latest_frame()
        print(f"📷 Camera source set to External USB Camera (Index {LAPTOP_CAMERA_INDEX})")
        return jsonify({"status": "success", "use_laptop_camera": True, "active_index": LAPTOP_CAMERA_INDEX})
    elif source == "laptop" or source == "hardware":
        USE_LAPTOP_CAMERA = True
        LAPTOP_CAMERA_INDEX = detect_best_camera_index()
        clear_latest_frame()
        print(f"📷 Camera source set to Auto/Hardware (Index {LAPTOP_CAMERA_INDEX})")
        return jsonify({"status": "success", "use_laptop_camera": True, "active_index": LAPTOP_CAMERA_INDEX})
    elif source == "tablet":
        USE_LAPTOP_CAMERA = False
        clear_latest_frame()
        print("📷 Camera source set to Tablet/Headset Camera")
...
def get_cameras():
    ...
    return jsonify({
        "available": available,
        "active": LAPTOP_CAMERA_INDEX
    })


@app.route("/api/switch_camera", methods=["POST"])
def switch_camera():
    global LAPTOP_CAMERA_INDEX
    available = get_all_available_cameras()
            
    if not available:
        return jsonify({"status": "error", "message": "No cameras detected"}), 400
        
    if len(available) <= 1:
        return jsonify({"status": "error", "message": "Only one camera available"}), 400
        
    try:
        current_pos = available.index(LAPTOP_CAMERA_INDEX)
    except ValueError:
        current_pos = -1
        
    next_pos = (current_pos + 1) % len(available)
    LAPTOP_CAMERA_INDEX = available[next_pos]
    clear_latest_frame()
    
    print(f"🔄 Camera switched to index {LAPTOP_CAMERA_INDEX}")
    return jsonify({
        "status": "success",
        "active": LAPTOP_CAMERA_INDEX,
        "name": f"Camera {LAPTOP_CAMERA_INDEX} ({'External USB' if LAPTOP_CAMERA_INDEX > 0 else 'Built-in'})"
    })
...
def capture_9gaze():
    ...
    # Map text directions to numeric filenames expected by NineGazeProcessing.py
    gaze_map = {
        "upleft": "1",
        "up": "2",
        "upright": "3",
...
@app.route("/api/preliminary/results", methods=["GET"])
def api_preliminary_results():
    global userName
    if not userName:
        return jsonify({"status": "error", "message": "No active patient session."}), 400
    try:
        images = sorted(
            [f for f in os.listdir("preliminary") if f.endswith((".jpg", ".png")) and f.startswith(userName)],
            key=lambda f: os.path.getmtime(os.path.join("preliminary", f)),
            reverse=True
        )
        if not images:
            return jsonify({"status": "error", "message": "No preliminary test images found."}), 404
        
        latest_image = images[0]
        text_filename = latest_image.replace(".jpg", ".txt").replace(".png", ".txt")
        text_path = os.path.join("Preliminary_Results", text_filename)
        ...
        return jsonify({
            ...
            "image_url": f"/download/preliminary/image/{latest_image}",
            "text_url": f"/download/preliminary/text/{text_filename}"
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route("/api/hirschberg/results", methods=["GET"])
def api_hirschberg_results():
    global userName
    if not userName:
        return jsonify({"status": "error", "message": "No active patient session."}), 400
    try:
        images = sorted(
            [f for f in os.listdir("HirschbergTestImages") if f.endswith((".jpg", ".png")) and f.startswith(f"hirschberg_{userName}")],
            key=lambda f: os.path.getmtime(os.path.join("HirschbergTestImages", f)),
            reverse=True
        )
        if not images:
            return jsonify({"status": "error", "message": "No Hirschberg test images found."}), 404
        
        latest_image = images[0]
        processed_filename = f"processed_{latest_image}"
        text_filename = processed_filename.replace(".jpg", ".txt").replace(".png", ".txt")
        text_path = os.path.join("Hirschberg_Results", text_filename)
        ...
        return jsonify({
            ...
            "text_file": text_filename,
            "text_content": text_content,
            "image_url": f"/download/hirschberg/image/{processed_filename}",
            "text_url": f"/download/hirschberg/text/{text_filename}"
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route("/api/9gaze/results", methods=["GET"])
def api_nine_gaze_results():
    global userName
    if not userName:
        return jsonify({"status": "error", "message": "No active patient session."}), 400
    try:
        ...
        # Robustly search for the patient folder matching userName
        import glob
        gaze_dirs = glob.glob(os.path.join(gaze_folder, f"*{userName}*"))
        gaze_dirs = [d for d in gaze_dirs if os.path.isdir(d)]
        if not gaze_dirs:
            return jsonify({"status": "error", "message": "No images found for this patient."}), 404
            
        person_folder = max(gaze_dirs, key=os.path.getctime)
        name = os.path.basename(person_folder)
        processed_folder = os.path.abspath(os.path.join(output_folder, f"{name}_Areal"))
        
        # Check if the test is already processed once
        text_path = os.path.join(processed_folder, "areal_ratios.txt")
        combined_image_filename = "combined_9gaze.jpg"
        combined_image_path = os.path.join(processed_folder, combined_image_filename)
        
        if os.path.exists(text_path) and os.path.exists(combined_image_path):
            processed_images = [f for f in os.listdir(processed_folder) if f.endswith(".jpg") and f != "combined_9gaze.jpg"]
            with open(text_path, "r") as f:
                text_content = f.read()
            return jsonify({
                ...
                "text_content": text_content,
                "combined_image_url": f"/9gaze_results/{name}/{combined_image_filename}",
                "processed_images_urls": [f"/9gaze_results/{name}/{img}" for img in processed_images]
            })
        ...
        if len(captured_images) < 9:
            return jsonify({
                "status": "error", 
                "message": f"Only {len(captured_images)}/9 images found. Test not complete."
            }), 400
        ...
        result_message, status_code = process_nine_gaze_images(name, gaze_folder, output_folder)
        if status_code != 200:
            return jsonify({"status": "error", "message": result_message}), status_code
...
@app.route("/api/edit_results", methods=["POST"])
def api_edit_results():
    global userName
    if not userName:
        return jsonify({"status": "error", "message": "No active patient session."}), 400
        
    test_type = request.form.get("test_type") # "preliminary", "hirschberg", or "nine_gaze"
    new_content = request.form.get("content")
    
    if not test_type or new_content is None:
        return jsonify({"status": "error", "message": "Missing test_type or content."}), 400
        
    try:
        if test_type == "preliminary":
            from overallreport import get_latest_match
            txt_path = get_latest_match("Preliminary_Results", ".txt", userName)
        elif test_type == "hirschberg":
            from overallreport import get_latest_match
            txt_path = get_latest_match("Hirschberg_Results", ".txt", userName)
        elif test_type == "nine_gaze":
            import glob
            gaze_output_folder = "9GazeResults"
            gaze_processed_dirs = glob.glob(os.path.join(gaze_output_folder, f"*{userName}*_Areal"))
            if gaze_processed_dirs:
                latest_dir = max(gaze_processed_dirs, key=os.path.getctime)
                txt_path = os.path.join(latest_dir, "areal_ratios.txt")
            else:
                txt_path = None
        else:
        ...
        if not txt_path or not os.path.exists(txt_path):
            return jsonify({"status": "error", "message": f"No existing results file found for {test_type}."}), 404
            
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(new_content)
            
        return jsonify({"status": "success", "message": "Results updated successfully."})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def api_status():
    global connected_device, stream_ip, userName, personDetails, camera_connected, USE_LAPTOP_CAMERA
    ...
    if userName:
        ...
        if not hirsch_files:
            # Fallback check
            hirsch_files = glob.glob(os.path.join("Hirschberg_Results", f"*_{userName}*.jpg"))
        if hirsch_files:
            hirschberg_status = True
            
        # Check 9Gaze Results
        gaze_dirs = glob.glob(os.path.join("9GazeResults", f"*{userName}*_Areal"))
        if gaze_dirs:
            nine_gaze_status = True
    ...
    return jsonify({
        ...
        "use_laptop_camera": USE_LAPTOP_CAMERA,
        "tests": {
            "preliminary": prelim_status,
            "hirschberg": hirschberg_status,
            "nine_gaze": nine_gaze_status
        }
    })
...
def api_intake():
    ...
    personDetails["id"] = patient_id
    
    # Format and sanitize
    dob_sanitized = patient_dob.replace("/", "_")
    raw_user_name = f"{patient_name}_{patient_id}_{dob_sanitized}"
    userName = re.sub(r'[\\/*?:"<>|]', '_', raw_user_name)
...
def api_reset():
    ...
    userName = False
    return jsonify({"status": "success", "message": "App state reset successfully."})
